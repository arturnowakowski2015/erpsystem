-- ============================================================================
-- Budget Enforcement Triggers - FIXED
...
BEGIN
  ...
  LOOP
    ...
    -- Get current budget to calculate new values
    SELECT achieved_amount INTO v_new_achieved
    FROM budgets
    WHERE id = v_line.budget_id;

    -- Calculate new achieved amount
    v_new_achieved := v_new_achieved + v_proportion;

    -- Credit the budget (increase achieved_amount for income budgets)
    UPDATE budgets
    SET 
      achieved_amount = v_new_achieved,
      remaining_balance = budgeted_amount - v_new_achieved,
      achievement_percentage = CASE 
        WHEN budgeted_amount > 0 THEN (v_new_achieved / budgeted_amount) * 100
        ELSE 0
      END,
      updated_at = NOW()
    WHERE id = v_line.budget_id;
...
BEGIN
  ...
  LOOP
    ...
    v_new_achieved := v_new_achieved + v_line.subtotal;
