...
import { Plus, Trash2, Package, Building, Calendar, Info, Receipt } from 'lucide-react';
...
export default function VendorBillForm() {
    ...
    const handleLineChange = useCallback(async (index: number, field: keyof BillLine, value: any) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const billData = {
                ...
                bill_number: formData.billNumber || `BILL-${format(new Date(), 'yyyyMMddHHmm')}`,
            ...
            // Handle lines
            if (!isNew) {
                await supabase.from('vendor_bill_lines').delete().eq('vendor_bill_id', billId);
            }
            ...
            toast.success(isNew ? 'Vendor bill created' : 'Vendor bill updated');
    ...
    return (
        <MainLayout>
            <PageHeader
                title={isNew ? 'New Vendor Bill' : 'Edit Vendor Bill'}
                subtitle="Create or modify vendor bill"
            />
