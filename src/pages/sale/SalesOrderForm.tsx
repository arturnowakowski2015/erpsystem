...
import { Plus, Trash2, Package, User, Calendar, Info } from 'lucide-react';
...
export default function SalesOrderForm() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isNew) {
                const newOrder = await useSalesOrderStore.getState().addOrder({
                ...
                // Handle lines (simple delete and re-insert for now)
                if (!isNew) {
                ...
                const { error: linesError } = await supabase.from('sales_order_lines').insert(
                    lines.map(l => ({
                        ...
                        // budget_id removed due to schema constraints
                    }))
                );

                if (linesError) throw linesError;

                await useSalesOrderStore.getState().fetchOrders();
            }
    ...
    return (
        <MainLayout>
            <PageHeader
                title={isNew ? 'New Sales Order' : 'Edit Sales Order'}
                subtitle="Create or modify customer sales order"
            />
