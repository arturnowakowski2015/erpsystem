...
-- Add foreign key for portal_contact_id
ALTER TABLE public.profiles ADD CONSTRAINT fk_portal_contact FOREIGN KEY (portal_contact_id) REFERENCES public.contacts(id);
...
-- Profiles policies
CREATE POLICY "Users can view own profile or admin can view all" ON public.profiles FOR SELECT USING (id = auth.uid() OR public.is_admin());
CREATE POLICY "Users can update own profile or admin can update all" ON public.profiles FOR UPDATE USING (id = auth.uid() OR public.is_admin());
...
CREATE POLICY "Portal users can view own purchase orders" ON public.purchase_orders FOR SELECT USING (vendor_id = public.get_portal_contact_id(auth.uid()));
...
CREATE POLICY "Portal users can view own PO lines" ON public.purchase_order_lines FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.purchase_orders po WHERE po.id = purchase_order_id AND po.vendor_id = public.get_portal_contact_id(auth.uid()))
);
...
CREATE POLICY "Portal users can view own vendor bills" ON public.vendor_bills FOR SELECT USING (vendor_id = public.get_portal_contact_id(auth.uid()));
...
CREATE POLICY "Portal users can view own bill lines" ON public.vendor_bill_lines FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.vendor_bills vb WHERE vb.id = vendor_bill_id AND vb.vendor_id = public.get_portal_contact_id(auth.uid()))
);
...
CREATE POLICY "Portal users can view own bill payments" ON public.bill_payments FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.vendor_bills vb WHERE vb.id = vendor_bill_id AND vb.vendor_id = public.get_portal_contact_id(auth.uid()))
);
...
CREATE POLICY "Portal users can view own sales orders" ON public.sales_orders FOR SELECT USING (customer_id = public.get_portal_contact_id(auth.uid()));
...
CREATE POLICY "Portal users can view own SO lines" ON public.sales_order_lines FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.sales_orders so WHERE so.id = sales_order_id AND so.customer_id = public.get_portal_contact_id(auth.uid()))
);

-- Customer invoices policies - Admins see ALL, portal users see their own
CREATE POLICY "Admins see all, portal users see their own" ON public.customer_invoices FOR SELECT USING (
  public.is_admin() OR customer_id = public.get_portal_contact_id(auth.uid()) OR user_id = auth.uid()
);
...
CREATE POLICY "Portal users can view own invoice lines" ON public.customer_invoice_lines FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.customer_invoices ci WHERE ci.id = customer_invoice_id AND ci.customer_id = public.get_portal_contact_id(auth.uid()))
);
...
CREATE POLICY "Portal users can view own invoice payments" ON public.invoice_payments FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.customer_invoices ci WHERE ci.id = customer_invoice_id AND ci.customer_id = public.get_portal_contact_id(auth.uid()))
);
